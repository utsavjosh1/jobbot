...
export const userRoleEnum = pgEnum("user_role", ["job_seeker", "employer", "admin", "discord_owner"]);
export const applicationStatusEnum = pgEnum("application_status", [
  "applied", "under_review", "phone_screen", "interviewed", "offer_extended", "accepted", "rejected", "withdrawn",
]);
export const subscriptionStatusEnum = pgEnum("subscription_status", ["active", "cancelled", "past_due", "trialing", "paused", "expired"]);
export const subscriptionPlanEnum = pgEnum("subscription_plan", ["seeker", "employer", "discord_owner"]);
export const paymentStatusEnum = pgEnum("payment_status", ["pending", "succeeded", "failed", "refunded", "disputed"]);
export const notificationStatusEnum = pgEnum("notification_status", ["pending", "sent", "failed", "bounced", "opened"]);
export const notificationChannelEnum = pgEnum("notification_channel", ["email", "push", "in_app"]);
export const botPlatformEnum = pgEnum("bot_platform", ["discord", "reddit", "twitter"]);
export const jobSourceEnum = pgEnum("job_source", [
  "remotive", "arbeitnow", "greenhouse", "hiring_cafe", "internal", "indeed", "linkedin", "company_direct", "generic",
]);
...
export const employer_profiles = pgTable("employer_profiles", {
  id: uuid("id").primaryKey().defaultRandom(),
  user_id: uuid("user_id").notNull().unique().references(() => users.id, { onDelete: "cascade" }),
...
export const jobs = pgTable("jobs", {
  ...
  remoteActiveIdx: index("idx_jobs_remote_active").on(table.remote, table.is_active).where(sql`${table.is_active} = true`),
  activePostedIdx: index("idx_jobs_active_posted").on(table.is_active, table.posted_at).where(sql`${table.is_active} = true`),
}));
...
export const job_matches = pgTable("job_matches", {
  id: uuid("id").primaryKey().defaultRandom(),
  user_id: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  resume_id: uuid("resume_id").notNull().references(() => resumes.id, { onDelete: "cascade" }),
...
export const applications = pgTable("applications", {
  id: uuid("id").primaryKey().defaultRandom(),
  seeker_id: uuid("seeker_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  ...
  seekerIdx: index("idx_applications_seeker").on(table.seeker_id, table.created_at),
  jobIdx: index("idx_applications_job").on(table.job_id, table.status),
}));

export const application_status_history = pgTable("application_status_history", {
  id: uuid("id").primaryKey().defaultRandom(),
  ...
  applicationIdx: index("idx_app_history_application").on(table.application_id, table.created_at),
}));
...
export const conversations = pgTable("conversations", {
  ...
  userActiveIdx: index("idx_conversations_user_active").on(table.user_id).where(sql`is_archived = false`),
}));
...
export const messages = pgTable("messages", {
  ...
  conversationIdx: index("idx_messages_conversation").on(table.conversation_id, table.created_at),
}));
...
export const bot_configs = pgTable("bot_configs", {
  id: uuid("id").primaryKey().defaultRandom(),
  user_id: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  ...
  activePlatformIdx: index("idx_bot_configs_active").on(t.platform, t.is_active).where(sql`${t.is_active} = true`),
}));

// DEPRECATED: Use bot_configs instead. Will be removed after migration.
export const discord_configs = pgTable("discord_configs", {
  id: uuid("id").primaryKey().defaultRandom(),
...
export const bot_posts = pgTable("bot_posts", {
  id: uuid("id").primaryKey().defaultRandom(),
  bot_config_id: uuid("bot_config_id").notNull().references(() => bot_configs.id, { onDelete: "cascade" }),
  ...
  configIdx: index("idx_bot_posts_config").on(table.bot_config_id, table.posted_at),
}));
...
export const subscription_change_log = pgTable("subscription_change_log", {
  id: uuid("id").primaryKey().defaultRandom(),
  user_id: uuid("user_id").notNull().references(() => users.id, { onDelete: "restrict" }),
  from_subscription_id: varchar("from_subscription_id", { length: 255 }),
  to_subscription_id: varchar("to_subscription_id", { length: 255 }).notNull(),
...
export const notifications = pgTable("notifications", {
  ...
  pendingIdx: index("idx_notifications_pending").on(table.user_id, table.status).where(sql`${table.status} = 'pending'`),
}));
...
export const audit_log = pgTable("audit_log", {
  ...
  entityIdx: index("idx_audit_entity").on(table.entity_type, table.entity_id, table.created_at),
}));
...
export const token_usage = pgTable("token_usage", {
  ...
  window_type: varchar("window_type", { length: 10 }).notNull().default("monthly"),
  tokens_used: integer("tokens_used").notNull().default(0),
}, (table) => ({
  userMonthIdx: uniqueIndex("idx_token_usage_monthly").on(table.user_id, sql`date_trunc('month', ${table.window_start})`),
}));
...
export const usersRelations = relations(users, ({ one, many }) => ({
  ...
  employer_profile: one(employer_profiles, { fields: [users.id], references: [employer_profiles.user_id] }),
  seeker_profile: one(seeker_profiles, { fields: [users.id], references: [seeker_profiles.user_id] }),
  seeker_applications: many(applications, { relationName: "seeker" }),
  subscription: one(subscriptions, { fields: [users.id], references: [subscriptions.user_id] }),
...
export const seekerProfilesRelations = relations(seeker_profiles, ({ one }) => ({
  user: one(users, { fields: [seeker_profiles.user_id], references: [users.id] }),
}));

export const employerProfilesRelations = relations(employer_profiles, ({ one, many }) => ({
  user: one(users, { fields: [employer_profiles.user_id], references: [users.id] }),
  jobs: many(jobs),
}));
...
export const jobMatchesRelations = relations(job_matches, ({ one }) => ({
  ...
  resume: one(resumes, { fields: [job_matches.resume_id], references: [resumes.id] }),
  job: one(jobs, { fields: [job_matches.job_id], references: [jobs.id] }),
}));

export const applicationsRelations = relations(applications, ({ one, many }) => ({
  seeker: one(users, { fields: [applications.seeker_id], references: [users.id], relationName: "seeker" }),
  job: one(jobs, { fields: [applications.job_id], references: [jobs.id] }),
  resume: one(resumes, { fields: [applications.resume_id], references: [resumes.id] }),
...
export const applicationStatusHistoryRelations = relations(application_status_history, ({ one }) => ({
  application: one(applications, { fields: [application_status_history.application_id], references: [applications.id] }),
}));

export const conversationsRelations = relations(conversations, ({ one, many }) => ({
  user: one(users, { fields: [conversations.user_id], references: [users.id] }),
  resume: one(resumes, { fields: [conversations.resume_id], references: [resumes.id] }),
...
export const botPostsRelations = relations(bot_posts, ({ one }) => ({
  config: one(bot_configs, { fields: [bot_posts.bot_config_id], references: [bot_configs.id] }),
...
export const planFeaturesRelations = relations(plan_features, ({ one }) => ({
  subscription: one(subscriptions, { fields: [plan_features.plan], references: [subscriptions.plan] }),
}));

export const subscriptionChangeLogRelations = relations(subscription_change_log, ({ one }) => ({
  user: one(users, { fields: [subscription_change_log.user_id], references: [users.id] }),
}));
